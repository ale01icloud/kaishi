...
def verify_token(token: str):
    ...
    try:
        ...
        chat_id_str, user_id_str, expires_at_str, signature = parts
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "显示机器人管理员":
        ...
        for admin in admins:
            ...
            lines.append(f"• {name} (@{username}){icon}")
    ...
    if text in ["删除机器人管理员", "移除机器人管理员"]:
        ...
        await update.message.reply_text(f"✅ 已移除 {target_user.first_name} 的管理员权限")
        return
    ...
    if text == "清除数据":
        ...
        if total_cleared == 0:
            ...
            lines = [
                "✅ 已清除今日数据（00:00至现在）\n",
                f"📥 已入账：清除 {in_count} 笔 ({float(in_usdt):.2f} USDT)",
                f"📤 已出账：清除 {out_count} 笔 ({float(out_usdt):.2f} USDT)",
                f"💰 已下发：清除 {send_count} 笔 ({float(send_usdt):.2f} USDT)",
            ]
            await update.message.reply_text("\n".join(lines))
    ...
    if text.startswith("+"):
        ...
        config = db.get_group_config(chat_id)
        rate = float(config.get("in_rate", 0) or 0)
        fx = float(config.get("in_fx", 0) or 0)

        if fx == 0:
            await update.message.reply_text("⚠️ 请先设置费率和汇率")
            return

        usdt = trunc2(amt * (1 - rate) / fx)
    ...
    if text.startswith("-"):
        ...
        config = db.get_group_config(chat_id)
        rate = float(config.get("out_rate", 0) or 0)
        fx = float(config.get("out_fx", 0) or 0)

        if fx == 0:
            await update.message.reply_text("⚠️ 请先设置费率和汇率")
            return

        usdt = round2(amt * (1 + rate) / fx)
...
def dashboard():
    ...
    config = db.get_group_config(chat_id)
    display_config = {
        "deposit_fee_rate": float(config.get("in_rate", 0) or 0) * 100,
        "deposit_fx": float(config.get("in_fx", 0) or 0),
        "withdrawal_fee_rate": float(config.get("out_rate", 0) or 0) * 100,
        "withdrawal_fx": float(config.get("out_fx", 0) or 0),
    }
...
def api_transactions():
    """获取今日交易记录（给前端 Dashboard）"""
    user_info = session.get("user_info")
    chat_id = user_info["chat_id"]

    transactions = db.get_today_transactions(chat_id)
    ...
    for txn in transactions:
        ...
        record = {
            ...
            "timestamp": txn["created_at"].timestamp()
    ...
    for record in records:
        ...
        if record["type"] == "deposit":
            ...
            stats["by_operator"][op]["deposit_usdt"] += record["usdt"]
        elif record["type"] == "withdrawal":
            stats["by_operator"][op]["withdrawal_count"] += 1
            stats["by_operator"][op]["withdrawal_usdt"] += record["usdt"]
        elif record["type"] == "disbursement":
            stats["by_operator"][op]["disbursement_count"] += 1
            stats["by_operator"][op]["disbursement_usdt"] += record["usdt"]
...
async def setup_telegram_bot():
    """构建 Telegram Application，注册 Handler，并设置 Webhook"""
    global telegram_app
    ...
    if WEBHOOK_URL:
        ...
        logger.warning("⚠️  WEBHOOK_URL 未设置，需要手动设置 webhook")
...
def init_app():
    ...
    if OWNER_ID and OWNER_ID.isdigit():
        ...
        logger.info(f"✅ OWNER已设置为管理员: {OWNER_ID}")
